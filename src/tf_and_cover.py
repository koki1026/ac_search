...
def update_coverage_map(coverage_map, position, area_size, coverage_radius=3):
    x, y = position
    # 船が通過したエリアに印をつける
    for i in range(max(0, int(x - coverage_radius)), min(area_size, int(x + coverage_radius) + 1)):
        for j in range(max(0, int(y - coverage_radius)), min(area_size, int(y + coverage_radius) + 1)):
            coverage_map[j][i] = 1

def calculate_coverage_rate(coverage_map):
    total_areas = np.prod(coverage_map.shape)
    covered_areas = np.sum(coverage_map)
    coverage_rate = covered_areas / total_areas
...
# カバレッジマップの初期化
coverage_map = np.zeros((area_size, area_size), dtype=int)
...
# カバレッジマップをプロット
plt.imshow(coverage_map, cmap='Blues', extent=[0, area_size, 0, area_size], alpha=0.5, origin='lower', interpolation='nearest', aspect='auto')
...
plt.xlabel('X-axis')
plt.ylabel('Y-axis')
plt.title('Trajectory with Navigation Task, Boundary Repulsion, and Coverage Map')
plt.legend()
plt.grid(True)
plt.show()
